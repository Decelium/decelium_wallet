...
const WS = require('ws');
const wrtc = require('wrtc'); // Require 'wrtc' instead of 'electron-webrtc'
...
function Peer(id, options) {
  ...
  //
    console.log("Peer() finished init.");
}
...
Peer.prototype._handleMessage = async function(message) {
    ...
    case 'OFFER': // we should consider switching this to CALL/CONNECT, but this is the least breaking option.
      ...
      if (connection) {
        ...
        for (var i = 0, ii = messages.length; i < ii; i += 1) {
          await connection.handleMessage(messages[i]);
        }
      }
      break;
    default:
...
Peer.prototype._delayedAbort = function(type, message) {
  console.log("Peer() FAILED ."+message);
...
Peer.prototype.listAllPeers = function(cb) {
  ...
  // If there's no ID we need to wait for one before trying to init socket.
  console.log("http: open 1");
  http.open('get', url, true);
  console.log("http: open 2");
  http.onerror = function(e) {
    self._abort('server-error', 'Could not get peers from the server.');
    cb([]);
  };
  console.log("http: open 3");
  http.onreadystatechange = function() {
    if (http.readyState !== 4) {
      return;
    }
